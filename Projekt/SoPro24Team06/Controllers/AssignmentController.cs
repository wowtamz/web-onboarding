...
namespace SoPro24Team06.Controllers
{
    ...
    public class AssignmentController : Controller
    {
        ...
        public async Task<IActionResult> EditAssignment([FromForm] AssignmentEditViewModel model)
        {
            ...
            {
                int processId = (int)TempData["detailProcessId"];
                TempData["detailProcessId"] = null;
                return Redirect($"/Process/Detail/{processId}");
            }
            ...
            if (TempData["editProcessViewModel"] != null)
            {
                ...
                return Redirect($"/Process/Edit/{editProcessViewModel.Id}");
            }
        ...
        [HttpGet("/Assignment/Edit/{assignmentId}")]
        public async Task<ActionResult> EditGetRequest(int assignmentId)
        {
            Assignment? assignment = _assignmentContainer.GetAssignmentById(assignmentId);
            if (assignment == null)
            {
                return NotFound("Required Assignment was not found");
            }
            if (TempData["detailProcessId"] != null || TempData["editProcessViewModel"] != null)
            {
                int processId = 0;
                string jsonViewModel = "";
                if (TempData["editProcessViewModel"] != null)
                {
                    jsonViewModel = TempData["editProcessViewModel"] as string;
                    EditProcessViewModel editProcessViewModel =
                        JsonConvert.DeserializeObject<EditProcessViewModel>(jsonViewModel);
                    processId = (int)editProcessViewModel.Id;

                    TempData["editProcessViewModel"] = JsonConvert.SerializeObject(
                        editProcessViewModel
                    );
                }

                if (TempData["detailProcessId"] != null)
                {
                    processId = (int)TempData["detailProcessId"];
                ...
                if (
                    ...
                    || (process != null && process.Supervisor == user)
                )
                {
